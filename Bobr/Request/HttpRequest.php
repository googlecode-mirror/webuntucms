...
class Bobr_Request_HttpRequest extends Object
{
	private $get = NULL;
	...
	{
		$this->get = new Bobr_Request_HttpGet;
     ...
     *
     * @return return Bobr_Request_HttpPost
     */
	...
	{
		$this->post = new Bobr_Request_HttpPost($_POST);
		unset($_POST);
        return $this;
	}
	...
	{
		$this->cookie = new Bobr_Request_HttpCookie;
	...
	{
       if (empty($_FILES)) {
           $this->files['empty'] = new Bobr_Request_HttpFile($_FILES);
       } else {
           foreach ($_FILES as $name => $file) {
                if (isset($file['name'])) {
                    $this->files[$name] = new Bobr_Request_HttpFile($_FILES[$name]);
                } else {
	...
	{
		$this->header = new Bobr_Request_HttpHeader();
		$this->header->assign();
        return $this;
	}
