...
abstract class ProcessWeb extends Request
{
	...
	// array pole hodnot, ktere ridi moduly
	protected $command = array();
	
	// string cesta k css
	protected $pageCss;
	// string cesta k template
	protected $pageTemplate;
	// string obsahuje cisla ( id bloku ) oddelene carkou
	protected $pageBlockIds;
	// integer cilo pageID
	protected $pageId;
	// array obsahuje vsechny dostupne pageId, jako klice pouziva ID
	protected $pageIdList = array();
	
	// array pole dostupnych jazyku
	protected $langList = array();
	// string jazyk ve kterem je stranka
	protected $lang;
	
	// instance BobrConfu
	protected $config;
	// prave pouzivana promena v getu
	protected $getVariable;
	// pokud jsou v getu spravne promene obsahuje TRUE a naopak
	 ...
	 * @return void
	 */
	protected function controlGetRequest()
	{
		$this->setRoot();
	...
	protected function qPage()
	{
		try{
			...
			if (FALSE === $this->isStaticUri( $this->GET[ $this->getVariable ] ) ){
				// zjistime jestli volani nejni dynamickeho modulu
				return $this->isDynamicUri( $this->GET[ $this->getVariable ] ) ;
			}else{
	...
	/**
	 * Podiva se do databaze jestli je v ni dana url
	 *
	...
	public function isStaticUri( $uri )
	{
		...
		if( count( $result ) ){
			...
			$this->lang			= $this->langList[ $result['lang_id'] ];
	...
	/**
	 * provadi kontrolu zda-li na poslednim miste v uri je lomitko, paklze ano tak ho orizne a presmeruje
	 * na url bez lomitka
	 * @param url
